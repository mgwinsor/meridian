...
**Status:** Active design baseline (v1.2)  
...
The first implementation therefore models only an `Account`. Concepts such as cash balances, currencies, countries, instruments, positions, portfolio groupings, valuations, and rebalancing rules remain deferred until the next vertical slice requires them.
...
`Account` is the only domain entity required for the first vertical slice.
...
## 7. Explicitly deferred product concepts

The following are part of the broader product direction but are **not part of the current domain model or first vertical slice**:
...
- cash balances;
- currency and country sentinel types;
- money value objects;
...
After the account vertical slice is complete, the next product story should determine the next domain concept.

A likely candidate is representing cash held in an account, but its model should be designed only when that story is selected. At that point, questions such as currency representation, whether a cash balance has independent identity, and how it relates to `Account` can be decided from the concrete requirement rather than predicted in advance.

## 9. MVP direction beyond the first slice
...
The exact sequence is intentionally not fixed beyond the first slice. Each new slice must earn any new domain concepts it introduces.
